...
func TestFinancialProviderRetrievesFullInvoiceAndRefund(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v1/invoices/in_1":
			_, _ = w.Write([]byte(`{"id":"in_1","customer":"cus_1","currency":"EUR","status":"paid","created":1700000000,"subtotal":1000,"total":1230,"amount_paid":1230,"amount_remaining":0,"metadata":{"account_id":"acc_1"},"parent":{"subscription_details":{"subscription":"sub_1"}},"total_taxes":[{"amount":230}]}`))
	...
	if invoice.AccountID != "acc_1" || invoice.SubscriptionID != "sub_1" || invoice.Tax != 230 || len(invoice.Lines) != 1 || invoice.Lines[0].PriceID != "price_1" {
